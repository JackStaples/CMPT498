...
class NavElem extends React.Component{
	...
	setSelected(e) {
		...
		if (this.state.currentTab === 0) {
			...
			ReactDOM.render(
				<Historical selected={this.state.selected} hexColumn={this.state.hexColumn} year="2016" test={this.state.column} date={moment(this.state.date)} dateTo={moment(this.state.dateTo)} dateFrom={moment(this.state.dateFrom)}/>,
				document.getElementById('container')
			);
			ReactDOM.render(
				<Refresh/>,
